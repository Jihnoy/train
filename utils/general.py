...
def draw_measure_line(xyxy, img, size, label, intrinsics_matrix, color):
    """
    :param xyxy:左上右下
    :param img:图像
    :param size 字体大小
    ...
    if label not in filter_list:
        ...
        if distance < 0:
            distance = "unknown"
        cv2.putText(img, str(distance) + 'm', (x + size, y), cv2.FONT_HERSHEY_SIMPLEX, fontScale=size, color=color,
                    thickness=3)
